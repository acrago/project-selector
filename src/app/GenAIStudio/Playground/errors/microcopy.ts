...
const fullFailureTemplates: Record<string, MicrocopyTemplate> = {
  ...
  'model:chat_template': {
    ...
    description: "{modelName} doesn't support the current chat template. Check the model's documentation for supported templates.",
  },
  'model:no_tools': {
    title: "This model doesn't support tool calling",
    description: "{modelName} doesn't support the tools feature you've enabled. Try selecting a model that supports tool calling, or disable tools in the Build panel.",
    actionSuggestion: 'Open Build panel',
  },
  'model:no_images': {
    title: "This model doesn't support image input",
    description: "{modelName} can't process images. Try selecting a multimodal model, or remove the image from your message.",
  },
  'llama_stack:timeout': {
...
const partialFailureTemplates: Record<string, MicrocopyTemplate> = {
  'rag:unreachable': {
    ...
    description: 'This response was generated without context from your knowledge sources. Results may be less accurate.',
  },
  'rag:embedding_failure': {
    title: 'Knowledge source retrieval failed',
    description: "The embedding model couldn't process your query for retrieval. The response doesn't include knowledge source context.",
  },
  'rag:no_results': {
    title: 'No matching knowledge found',
    description: "Your knowledge sources didn't return any relevant results. The response was generated without additional context.",
  },
  ...
  'mcp:unreachable': {
    ...
    description: "The model attempted to use the {toolName} tool but the server didn't respond. The response was generated without this tool's output.",
  },
  'mcp:auth_failure': {
    title: '{toolName} tool call failed',
    description: "The {toolName} tool server rejected the request due to an authentication error. Check the server's credentials in the Build panel.",
    actionSuggestion: 'Open Build panel',
  },
  'mcp:execution_error': {
    title: '{toolName} tool returned an error',
    description: 'The {toolName} tool encountered an error during execution. The response may be incomplete.',
  },
};
...
const streamingTemplates: Record<string, MicrocopyTemplate> = {
  ...
  'stream:timeout': {
    ...
    description: 'The model stopped responding during generation. The response above is incomplete.',
  },
