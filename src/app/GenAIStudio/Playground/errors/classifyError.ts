import { ApiError, ClassifiedError } from './types';
...
function resolveTemplateKey(error: ApiError): string {
  ...
  if (error.status === 429) return 'bff:rate_limit';
  if (error.status && error.status >= 500) return 'bff:network_error';
  if (error.status === 0 || (!error.status && !component)) return 'bff:unreachable';

  return 'unknown';
}

function resolveRetriable(error: ApiError): boolean {
  if (error.error?.retriable !== undefined) return error.error.retriable;

  const code = error.error?.code;
  if (code && RETRIABLE_CODES.has(code)) return true;
  if (error.status && RETRIABLE_STATUSES.has(error.status)) return true;

  // Fallback: unknown errors are retriable per spec
  const templateKey = resolveTemplateKey(error);
  if (templateKey === 'unknown') return true;
...
export function classifyError(
  ...
  const displayComponent = component
    ...
    : 'Unknown';
  const componentLabel = component === 'mcp' && effectiveContext.toolName
